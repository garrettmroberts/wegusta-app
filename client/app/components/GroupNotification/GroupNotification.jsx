...
const GroupNotification = ({ users, unread, lastUpdated, onPress }) => {
    ...
    const generateAvatars = users => {
        ...
        if (users.length > 2) {
            avatars.push(
                <View
                    style={styles.additionalUsersAvatar}
                    key={'additionalUsers'}
                >
    ...
    const generateAccessory = (unread, lastUpdated) => {
        if (unread) {
            ...
            const month = lastUpdated.toLocaleString('default', {
                month: 'long',
            })
            const date = formatNumber(lastUpdated.getDate())
            return (
                <Text style={styles.lastUpdatedText}>{`${month} ${date}`}</Text>
            )
        }
    }
    ...
    return (
        <Pressable
            style={[styles.groupNotifications, wrapperClass(unread)]}
            onPress={onPress}
        >
